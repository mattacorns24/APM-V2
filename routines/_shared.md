...
All state is in the **private data repo** mounted/cloned at `output/`:

1. Start: if `output/.git` exists → `git -C output pull --rebase`. If `output/` is missing or not a git repo, clone the private data repo `apm-v2-data` into `output/`.
